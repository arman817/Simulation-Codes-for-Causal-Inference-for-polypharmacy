...
Propensity_Score_Function1 <- function(Y,R,pi){
  data1 <- data.frame(R,pi,Y)
  model <- glm(Y~.,data = data1, family = "binomial")
  data2 <- data.frame(R=1,pi)
  Prop_Adj <- predict.glm(model,type="response",newdata = data2)
...
#Removal of the least 20% of the regimens in the dataset
Trunc_Data1 <- function(data_obtain,n){
  data_obtain <- data.frame(data_obtain)
  Site <- data_obtain[,1]
  len <- length(unique(Site))                     #Number of Classes
  Trunc_Size <- floor(length(unique(Site))*0.8)   #No. of classes left after truncation
  while(len > Trunc_Size){
    First_Remove_Class <- min(data_obtain$Freq)
    Instances <- sum(data_obtain$Freq == First_Remove_Class)
    if(Instances <= (len-Trunc_Size)){
      data_obtain <- data_obtain[which(data_obtain$Freq != First_Remove_Class),]
      len <- len - Instances
    }
    else{
      sum <- 0
      for(i in 1:n){
        if((sum<(len-Trunc_Size)) && (data_obtain$Freq[i] == First_Remove_Class)){
          data_obtain$Freq[i] <- 0
          sum <- sum + 1
        }
      }
      data_obtain <- data_obtain[which(data_obtain$Freq != 0),]
      len <- Trunc_Size
    }
  }
  newdata <- data_obtain
...
#Removal of the least 30% of the regimens in the dataset
Trunc_Data2 <- function(data_obtaine,n){
  data_obtaine <- data.frame(data_obtaine)
  Site <- data_obtaine[,1]
  len <- length(unique(Site))                     #Number of Classes
  Trunc_Size <- floor(length(unique(Site))*0.7)   #No. of classes left after truncation
  while(len > Trunc_Size){
    First_Remove_Class <- min(data_obtaine$Freq)
    Instances <- sum(data_obtaine$Freq == First_Remove_Class)
    if(Instances <= (len-Trunc_Size)){
      data_obtaine <- data_obtaine[which(data_obtaine$Freq != First_Remove_Class),]
      len <- len - Instances
    }
    else{
      sum <- 0
      for(i in 1:n){
        if((sum<(len-Trunc_Size)) && (data_obtaine$Freq[i] == First_Remove_Class)){
          data_obtaine$Freq[i] <- 0
          sum <- sum + 1
        }
      }
      data_obtaine <- data_obtaine[which(data_obtaine$Freq != 0),]
      len <- Trunc_Size
    }
  }
  newdata <- data_obtaine
  return(newdata)
}
...
SVM_Value <- function(data_obt,Original_Data,n){
  ...
  #Creating a model for the subsetted data
  model <- svm(Classes ~ ., data = MainData)
  x <- subset(MainData, select = -Classes)
  y <- Classes
  model <- svm(x, y, probability = TRUE)
  
  #Predicting probabilities for the entire data
...
Softmax_Regression_Value <- function(data_obt,Original_Data,n){
  ...
  #Predicting probabilities for the entire data using the function predict.softmax1 defined earlier
...
Twang_Package_Value <- function(data_obta,n){
  data_obta <- data.frame(data_obta)
  R_1 <- data_obta[,25]
  Y <- data_obta[,24]
  G_comp1 <- data_obta[,29]
  G_comp2 <- data_obta[,26]
  Medication <- data_obta[,16:23]
  Cov <- data_obta[,2:15]
  data_req <- data.frame(data_obta)
  
  #Creating a GBM for Regimen 1
  ps.1 <- ps(R_1 ~ X_1+X_2+X_3+X_4+X_5+X_6+X_7+X_8+X_9+X_10+X_11+X_12+X_13+X_14,
             data = data_req,
...
for(p in start:end){
  ...
  outpath=paste("/home/arman13/Medication/Output/Error_Re/Third/Sim_",start,"_",end,".txt",sep="")
